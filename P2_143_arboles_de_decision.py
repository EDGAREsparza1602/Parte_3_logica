...
def calcular_ganancia(datos, atributo):
    ...
    for valor in valores:
        datos_subset = datos[datos[:, atributo] == valor]
        entropia_atributo += peso_valores[np.where(valores == valor)[0][0]] * calcular_entropia(datos_subset)
...
def construir_arbol(datos):
    ...
    for valor in valores_atributo:
        ...
        nodo.hijos[valor] = construir_arbol(datos_subset)
